...
void DALS::CalcALCs(const std::vector<ObjPtr> &target_nodes) {
    ...
    std::unordered_map <ObjPtr, LAC_t> opt_LAC;
    SASIMI_CalcALCs(target_ntk_, approx_ntk_,opt_LAC);
    for (auto const &t_node : target_nodes) {
    	ALC alc(t_node, opt_LAC[t_node].GetSS(), opt_LAC[t_node].GetIsInv(),opt_LAC[t_node].GetDError());
...
void DALS::Run(double err_constraint) {
	...
	auto critical=GetCriticalGraph(approx_ntk_);
