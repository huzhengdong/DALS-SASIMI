...
// evaluate
void Ckt_EvalASIC(Abc_Ntk_t * pNtk, std::string fileName, double maxDelay, bool isOutput)
{
    std::string Command;
    std::string resyn2 = "strash; balance; rewrite; refactor; balance; rewrite; rewrite -z; balance; refactor -z; rewrite -z; balance;";
    float area, delay;

    Abc_Frame_t * pAbc = Abc_FrameGetGlobalFrame();
    Abc_FrameReplaceCurrentNetwork(pAbc, Abc_NtkDup(pNtk));

    Command = resyn2 + std::string("amap;");
    ...
    if (delay >= maxDelay + 0.1)
        std::cout << "Warning: exceed required delay" << std::endl;
...
void Ckt_EvalASIC(IN Abc_Ntk_t * pNtk, IN string && fileName, IN double maxDelay, OUT float & area, OUT float & delay)
{
    ...
    std::string resyn2 = "strash; balance; rewrite; refactor; balance; rewrite; rewrite -z; balance; refactor -z; rewrite -z; balance;";
    std::string Command = resyn2 + string("amap;");
    DASSERT(!Cmd_CommandExecute(pAbc, Command.c_str()));
...
{
    const std::string resyn2 = "strash; balance; rewrite; refactor; balance; rewrite; rewrite -z; balance; refactor -z; rewrite -z; balance;";

    Abc_Frame_t * pAbc = Abc_FrameGetGlobalFrame();
    Abc_FrameReplaceCurrentNetwork(pAbc, Abc_NtkDup(pNtk));
    std::string Command = resyn2 + map;
...
float Ckt_GetDelay(Abc_Ntk_t * pNtk)
{
    DASSERT(Abc_NtkHasMapping(pNtk));
    int i=0;
    abc::Abc_Obj_t * pt=NULL;
    //Abc_Obj_t * pn=NULL;
    
    return abc::Abc_NtkDelayTrace(pNtk, pt, pt, i );
}
...
{
    std::cout << tsName << "," << ssName << std::endl;
