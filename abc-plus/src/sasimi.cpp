...
void SASIMI_Manager_t::CalcLAC(Abc_Ntk_t * pOriNtk, Abc_Ntk_t * PAppNtk, unordered_map <Abc_Obj_t *, LAC_t> opt_alc)
{
    // init
    Abc_Ntk_t * pAppNtk = Abc_NtkDup(PAppNtk);
    PatchConst(pOriNtk);
    PatchConst(pAppNtk);
    ...
    //while (error < errorBound) {
        ...
        {
	   opt_alc.emplace(nodeLAC.GetTS(), nodeLAC);
	}
...
/*namespace abc_plus {
void SASIMI_CalcALCs(const std::vector<ObjPtr> &target_nodes,NtkPtr target_ntk_, NtkPtr approx_ntk_,std::unordered_map <Abc_Obj_t *, LAC_t> opt_alc_) {
    Metric_t metricType = Metric_t::ER;
    std::unordered_map <Abc_Obj_t *, LAC_t> opt_LAC;
    ...
    for (auto const &t_node : target_nodes) {
    	LAC_t alc(t_node, opt_LAC[t_node].GetSS(), opt_LAC[t_node].GetIsInv(),opt_LAC[t_node].GetDError());
        opt_alc_.emplace(t_node, alc);  
    }
}
}*/
